...
CondDBRef_EMAP = CondDB.clone( connect = cms.string( 'sqlite_file:GEMELMap_20180929.db' ) )
CondDBRef_EMAP.DBParameters.messageLevel = cms.untracked.int32( options.messageLevel )

process.GEMEMap_SQLiteSource = cms.ESSource( "PoolDBESSource",
                                   CondDBRef_EMAP,
                                   toGet = cms.VPSet( cms.PSet( record = cms.string('GEMELMapRcd'),
...
process.reader_elmap = cms.EDAnalyzer( "GEMELMapDBReader",
                                 dumpFileName = cms.untracked.string( "dumpELMap-from-sqlite-v3.out" )
)


#Path definition
process.GEMQC8ConfReaderSourcePath = cms.Path( process.reader + process.recordDataGetter + process.reader_elmap )
